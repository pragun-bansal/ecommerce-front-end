...
const RatingGraph = ({product}) => {
    ...
    const ratingWith1 = product.reviews.reduce((count, review) => {
      ...
      const percent5 =(
        ...
        const totalStar = (((ratingWith1*1)+(ratingWith2*2)+(ratingWith3*3)+(ratingWith4*4)+(ratingWith5*5))/product.reviews.length).toFixed(2)
        const stars = Math.round(totalStar)
    return (
        <div className='lg:w-[50vw] ml-4 mt-16'>
            ...
            {[...Array(5-stars)].map((e, i) => (
                ...
                <p class="ml-2 text-sm font-medium text-gray-400 dark:text-gray-500">{totalStar} out of 5</p>
            </div>
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{product.reviews.length} global ratings</p>
